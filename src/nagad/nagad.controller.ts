...
// 	}
  ...
  async createPayment(@Req() req:Request, @Res() res:Response){
      ...
      const mar_pri_keys =`-----BEGIN PRIVATE KEY-----\n${pgprivateKey}\n-----END PRIVATE KEY-----`;
		const mar_pri_key = new NodeRSA(mar_pri_keys);
		const signerObject = crypto.createSign('RSA-SHA1');
		signerObject.update(JSON.stringify(sensitiveData),);
		const digitalSignature = signerObject.sign(mar_pri_keys,'base64');

		const encryptedSensitiveData =crypto.publicEncrypt(
			{ key: publicKey, padding: crypto.constants.RSA_PKCS1_PADDING },
			Buffer.from(JSON.stringify(sensitiveData), "base64")
		 );
		console.log(encryptedSensitiveData);
		const decryptedData = crypto.privateDecrypt(
			{
			  key: mar_pri_keys,
			  padding: crypto.constants.RSA_PKCS1_PADDING
			},
			Buffer.from(JSON.stringify(encryptedSensitiveData),"base64")
		 );
			...
			if (response.data.sensitiveData !== "" && response.data.signature !== "") {
		      ...
		      const mar_pri_key =`-----BEGIN PRIVATE KEY-----\n${pgprivateKey}\n-----END PRIVATE KEY-----`;
				...
				const sensitiveOrderData = {
				 ...
				 const encryptedOrderData = pubKey.encrypt(JSON.stringify(sensitiveOrderData), 'base64');
   ...
   const mar_pri_key =`-----BEGIN PRIVATE KEY-----\n${pgprivateKey}\n-----END PRIVATE KEY-----`;
	...
	private generateSignature(data: string | Record<string, string>) {
		const pgprivateKey =' '
      const mar_pri_key =`-----BEGIN PRIVATE KEY-----\n${pgprivateKey}\n-----END PRIVATE KEY-----`;
