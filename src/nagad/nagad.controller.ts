...
export class NagadController {
     ...
     constructor(config: INagadConstructor,    private readonly nagadGatewayService: NagadGatewayService,) {
 ...
 private async  get<T>(url: string, additionalHeaders: IHeaders): Promise<any> {
	...
	return data;
     ...
     private async post<T>(url: string, payload: IPayload = {}, additionalHeaders: IHeaders): Promise<T> {
     ...
     private async confirmPayment(data: IConfirmPaymentArgs, clientType: IClientType): Promise<INagadPaymentURL> {
        ...
        return await this.post<INagadPaymentURL>(`${this.baseURL}/api/dfs/check-out/complete/${paymentReferenceId}`, payload, {
           ...this.headers,
           'X-KM-IP-V4': newIP,
           'X-KM-Client-Type': clientType,
        });
